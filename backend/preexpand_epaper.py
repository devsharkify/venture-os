...
async def expand_one(article):
    """Expand a single article summary."""
    cache_key = "epaper_summary_en"
    ...
    try:
        expanded = await expand_summary_for_epaper(title, summary, article.get("category", ""), "en")
...
async def main():
    ...
    articles = await db.news.find(
        ...
        {"_id": 0, "id": 1, "title": 1, "summary": 1,
         "category": 1, "epaper_summary_en": 1}
    ).to_list(5000)
    ...
    for i in range(0, len(en_need), BATCH):
        ...
        results = await asyncio.gather(*[expand_one(a) for a in batch], return_exceptions=True)
    ...
    print(f"TOTAL: {en_done} articles expanded")
