...
export const EpaperToolbar = ({ currentPage, totalPages, zoom, setZoom, goPage, selectedDate, selectedSlot, setSelectedSlot, showDatePicker, setShowDatePicker, editions, fmtDate, shareEpaper, downloadPdf, pdfLoading, slotsForDate, selectEdition }) => (
  <div className="flex items-center justify-center gap-2 px-3 py-1.5 flex-shrink-0 flex-wrap" style={{ background: "#222", borderBottom: "1px solid #444" }}>
    <button onClick={() => goPage(-1)} disabled={currentPage === 0} className="p-1 text-gray-400 disabled:opacity-30 hover:text-white" data-testid="epaper-prev-page"><ChevronLeft size={16} /></button>
    <span className="text-xs text-gray-300 font-mono">Pg <strong className="text-white">{currentPage + 1}</strong>/{totalPages}</span>
    <button onClick={() => goPage(1)} disabled={currentPage >= totalPages - 1} className="p-1 text-gray-400 disabled:opacity-30 hover:text-white" data-testid="epaper-next-page"><ChevronRight size={16} /></button>
    ...
    <div className="w-px h-5 bg-gray-600" />
    <button onClick={() => setZoom(z => Math.max(0.2, z - 0.05))} className="p-1 text-gray-400 hover:text-white" data-testid="epaper-zoom-out"><ZoomOut size={14} /></button>
    <span className="text-[10px] text-gray-400 w-8 text-center font-mono">{Math.round(zoom * 100)}%</span>
    <button onClick={() => setZoom(z => Math.min(1, z + 0.05))} className="p-1 text-gray-400 hover:text-white" data-testid="epaper-zoom-in"><ZoomIn size={14} /></button>
    <button onClick={() => setZoom(0.38)} className="p-1 text-gray-400 hover:text-white"><Maximize2 size={13} /></button>
    <div className="w-px h-5 bg-gray-600" />
    <div className="relative">
      <button data-testid="epaper-date-picker" onClick={() => setShowDatePicker(!showDatePicker)} className="flex items-center gap-1 px-2 py-1 text-xs text-gray-300 rounded hover:bg-gray-700 border border-gray-600">
        <Calendar size={11} />{fmtDate(selectedDate)}
      </button>
      {showDatePicker && (
        <div className="absolute right-0 top-full mt-1 z-50 max-h-64 overflow-y-auto w-64 shadow-xl rounded bg-gray-900 border border-gray-700">
          {editions.map(e => {
            ...
            return (
              <button key={`${e.date}_${e.slot}`} onClick={() => { selectEdition(e.date, e.slot); setShowDatePicker(false); }}
                className="w-full text-left px-3 py-2 text-[11px] flex justify-between items-center hover:bg-gray-800 border-b border-gray-800"
                ...
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-gray-500">{e.article_count}</span>
                </span>
              </button>
            );
          })}
    ...
    <button data-testid="epaper-share" onClick={shareEpaper} className="flex items-center gap-1 px-2 py-1 text-xs text-gray-300 rounded hover:bg-gray-700 border border-gray-600">
