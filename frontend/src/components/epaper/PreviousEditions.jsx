...
export const PreviousEditions = ({ recentDates, editions, olderByMonth, selectedDate, selectedSlot, selectEdition, fmtDate }) => {
  ...
  return (
    <div style={{ background: "#1a1a1a", borderTop: "4px solid #2D7AFF" }} data-testid="epaper-archive">
      <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "28px 20px 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
          <Archive size={20} style={{ color: "#2D7AFF" }} />
          <h2 style={{ fontSize: "22px", fontWeight: 800, color: "#fff", fontFamily: "'Syne', Georgia, serif", letterSpacing: "0.5px" }}>
            Previous Editions
          </h2>
        </div>
        <p style={{ fontSize: "13px", color: "#888", marginBottom: "20px" }}>
          Browse past morning and evening editions of Venture OS ePaper
        </p>
          ...
          {recentDates.map(date => {
            ...
            return (
              <div key={date} data-testid={`archive-card-${date}`}
                style={{ background: "#2a2a2a", border: selectedDate === date ? "2px solid #2D7AFF" : "1px solid #3a3a3a", borderRadius: "8px", padding: "14px 10px", textAlign: "center" }}>
                <div style={{
                  width: "44px", height: "44px", borderRadius: "50%",
                  background: selectedDate === date ? "rgba(196,30,30,0.2)" : "#333",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  margin: "0 auto 8px", fontSize: "20px", fontWeight: 900,
                  color: selectedDate === date ? "#2D7AFF" : "#ccc", fontFamily: "'Syne', serif",
                }}>{getDayNum(date)}</div>
                    ...
                    return (
                      <button key={s} data-testid={`archive-${date}-${s}`}
                        ...
                        style={{
                          ...
                          background: isActive ? "#2D7AFF" : ed ? "#3a3a3a" : "#2a2a2a",
        ...
        {months.length > 0 && (
          <div style={{ borderTop: "1px solid #333", paddingTop: "16px", paddingBottom: "24px" }}>
            <button data-testid="archive-show-older" onClick={() => setShowOlder(!showOlder)}
              style={{ display: "flex", alignItems: "center", gap: "8px", background: "none", border: "none", cursor: "pointer", color: "#2D7AFF", fontSize: "14px", fontWeight: 700, padding: "0", marginBottom: showOlder ? "16px" : "0" }}>
                      ...
                      {olderByMonth[month].map(e => {
                        ...
                        return (
                          <button key={`${e.date}_${e.slot}`} data-testid={`archive-link-${e.date}-${e.slot}`}
                            ...
                            style={{
                              background: isActive ? "#2D7AFF" : "transparent",
                              border: `1px solid ${isActive ? "#2D7AFF" : "#444"}`,
