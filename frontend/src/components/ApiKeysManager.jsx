    ...
    <div data-testid="api-keys-manager" className="space-y-4">
      ...
      <div className={`rounded-xl border p-5 ${bg}`}>
        <h3 className={`text-sm font-bold mb-3 flex items-center gap-2 ${txt}`}>
          <Key className="w-4 h-4 text-[#2D7AFF]" /> Create API Key
        </h3>
        <div className="flex gap-2 items-end">
          ...
          <button
            ...
            className="px-4 py-2 bg-[#2D7AFF] text-white rounded-lg text-sm font-semibold hover:bg-[#1A5FCC] disabled:opacity-50 flex items-center gap-1"
          >
        ...
        {createdKey && (
          <div className="mt-3 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
            <p className="text-xs font-bold text-green-700 dark:text-[#00D9C8] mb-1">API Key Created. Save it now!</p>
      ...
      <div className={`rounded-xl border ${bg}`}>
        <div className="p-4 border-b border-[#131B2A] dark:border-[#1C2840]">
              ...
              <div key={k.id} data-testid={`api-key-${k.id}`} className="p-4 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-1.5 py-0.5 text-[10px] font-medium rounded ${k.active ? "bg-green-100 text-green-700 dark:bg-[#00D9C8]/10 dark:text-[#00D9C8]" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={() => copyToClipboard(exampleCode, "code")}
                className={`text-[10px] flex items-center gap-1 px-2 py-1 rounded ${sub} hover:bg-[#131B2A] dark:hover:bg-[#131B2A]`}>
                {copied === "code" ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />} Copy
              </button>
            </div>
            <pre className={`p-3 rounded-lg text-[11px] font-mono leading-relaxed overflow-x-auto ${darkMode ? "bg-[#070B12] text-[#A0B4CC]" : "bg-[#0A0E18] text-[#A0B4CC]"}`}>
              {exampleCode}
            </pre>
          </div>
